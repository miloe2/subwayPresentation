...
const OpenMintimeModal = ({ estimatedTime2,selectedStations2}) => {
    // const [clickStyle, setClickStyle] = useState();

    return(
        <Container>
            <div className='minTime'>
                <div className ='estimateTime'> <p> 최소시간 <br/> <span>{estimatedTime2}분 </span> </p> </div>
                
                {selectedStations2 && (
                    <ul>
                        {selectedStations2.map((station, index) => {
                            if (index === 0 || station.line !== selectedStations2[index - 1].line || index === selectedStations2.length - 1) {
                            ...
                            return (
                                <li key={index} style={{borderColor: lineColor}} >
                                    ...
                                    {/* 마지막 요소의 라인 삭제 */}
                                    {index !== selectedStations2.length - 1 && <div className='hrLine' style={{ borderColor: lineColor}}> 
...
export default OpenMintimeModal;